# References

## Links gathered from the internet

### Linux Tools

- [Parted Magic](https://partedmagic.com/)

### Manjaro’s Community

- [Manjaro Wiki](https://wiki.manjaro.org/index.php/Main_Page)
- [Manjaro Linux Forum](https://forum.manjaro.org/)
- [Add Certificated](https://warlord0blog.wordpress.com/2021/01/17/trusting-ca-certificates-manjaro/)

### ZSH Plugins

- [Oh-My-Zsh Plugin Manager](https://ohmyz.sh/)
- [LSD (LSDeluxe)](https://github.com/lsd-rs/lsd)
- [Zsh-autosuggestions](https://github.com/zsh-users/zsh-autosuggestions/blob/master/INSTALL.md)
- [Zsh-syntax-highlighting](https://github.com/zsh-users/zsh-syntax-highlighting/blob/master/INSTALL.md)
- [FZF - Command-line Fuzzy Finder](https://github.com/junegunn/fzf)
- [Zsh-interactive-cd](https://github.com/mrjohannchang/zsh-interactive-cd?tab=readme-ov-file)

### Zsh Themes

- [PowerLevel 10K](https://github.com/romkatv/powerlevel10k?tab=readme-ov-file#oh-my-zsh)

### Pipewire

- [Pipewire ArchLinux Wiki](https://wiki.archlinux.org/title/PipeWire)
- [Pipewire](https://pipewire.org/)
- [Pipewire Docs](https://docs.pipewire.org/)

### Wireplumber

- [Wireplumber ArchLinux Wiki](https://wiki.archlinux.org/title/WirePlumber)
- [Wireplumber](https://pipewire.pages.freedesktop.org/wireplumber/index.html)

### Python

- [Python](https://www.python.org/)
- [Python Virtualenv](https://virtualenv.pypa.io/en/latest/)
- [Python Virtualenv Docs](https://virtualenvwrapper.readthedocs.io/en/latest/)

### AmdFan

- [AmdFan](https://github.com/mcgillij/amdfan)

### HeadsetControl

- [HeadsetControl](https://github.com/Sapd/HeadsetControl)

### Mesa

- [Mesa](https://www.mesa3d.org/)
- [Mesa ArchLinux Wiki](https://wiki.archlinux.org/title/OpenGL)

### Vulkan

- [Vulkan](https://www.vulkan.org/)
- [Vulkan ArchLinux Wiki](https://wiki.archlinux.org/title/Vulkan)

### Benchmarking

- [Benchmarking ArchLinux Wiki](https://wiki.archlinux.org/title/Benchmarking)

### Citrix Workspace

- [ICAClient](https://aur.archlinux.org/packages/icaclient)
- [Citrix ArchLinux Wiki](https://wiki.archlinux.org/title/citrix)
- [Citrix App Protection](https://docs.citrix.com/en-us/citrix-virtual-apps-desktops/secure/app-protection.html)

### Waydroid

- [Waydroid](https://waydro.id/)
- [Waydroid ArchLinux Wiki](https://wiki.archlinux.org/title/Waydroid)
- [Device is not Play Protect certified](https://github.com/waydroid/waydroid/issues/753)
- [Device is not Play Protect certified](https://github.com/casualsnek/waydroid_script/issues/26)
- [Android device uncertified](https://www.google.com/android/uncertified/)

### Java

- [Java](https://www.java.com/)
- [Java ArchLinux Wiki](https://wiki.archlinux.org/title/Java)

### PhpMyAdmin

- [PhpMyAdmin](https://www.phpmyadmin.net/)
- [PhpMyAdmin Docs](https://www.phpmyadmin.net/docs/)
- [PhpMyAdmin ArchLinux Wiki](https://wiki.archlinux.org/title/PhpMyAdmin)
- [PhpMyAdmin Install Guide](https://techviewleo.com/install-phpmyadmin-on-arch-manjaro-garuda-linux/)

### Flatpak

#### Bottles

- [Bottles](https://usebottles.com)
- [Bottles Docs](https://docs.usebottles.com)

### Proton

- [ProtonUp-NG](https://github.com/protonup-ng/protonup-ng)

### Goverlay

- [Goverlay](https://github.com/benjamimgois/goverlay)

### Replay Sorcery

- [Replay Sorcery](https://github.com/matanui159/ReplaySorcery)

### Lutris

- [Lutris](https://lutris.net/)

### Wine

- [Wine](https://www.winehq.org/)
- [Wine ArchLinux Wiki](https://wiki.archlinux.org/title/Wine)

### Winetricks

- [Winetricks](https://wiki.winehq.org/Winetricks)
- [Winetricks ArchLinux Wiki](https://wiki.archlinux.org/title/Winetricks)

### GSConnect & KDEConnect

- [GSConnect Guide](https://askubuntu.com/questions/1036351/how-to-install-and-run-gsconnect-to-have-android-integration-transfer-files-se)
- [GSConnect ArchLinux Wiki](https://wiki.archlinux.org/title/GSConnect)
- [KDEConnect](https://userbase.kde.org/KDEConnect#)

### Printer Drivers

- [Printer Drivers](https://wiki.archlinux.org/title/CUPS/Printer-specific_problems)
- [CUPS](https://wiki.manjaro.org/index.php/Printing#Managing_Printers_with_CUPS)

### Kernel

- [Kernel ArchLinux Wiki](https://wiki.archlinux.org/title/Kernel)
- [Kernel](https://www.kernel.org/)
- [Kernel Newbies](https://kernelnewbies.org/)
- [Kernel Source](https://www.kernel.org/doc/html/latest/index.html)
- [Manjaro Kernels Wiki](https://wiki.manjaro.org/index.php/Manjaro_Kernels#Overview)

### DNS

- [Cloudflare](https://www.cloudflare.com/)
- [Cloudflare Docs](https://developers.cloudflare.com/
- [Cloudflare Setup](https://developers.cloudflare.com/1.1.1.1/setup/linux/)
- [Google](https://developers.google.com/speed/public-dns/docs/using)

### Git

- [Git](https://git-scm.com/)
- [Git ArchLinux Wiki](https://wiki.archlinux.org/title/Git)
- [Git Docs](https://git-scm.com/doc)
- [Git Book](https://git-scm.com/book/en/v2)
- [Git Cheat Sheet](https://training.github.com/downloads/github-git-cheat-sheet.pdf)
- [Git Tutorial](https://www.atlassian.com/git/tutorials)
- [Git Dubious Ownership Error](https://confluence.atlassian.com/bbkb/git-command-returns-fatal-error-about-the-repository-being-owned-by-someone-else-1167744132.html)

### Local AI

- [Local AI](https://en.wikipedia.org/wiki/Local_AI)
- [ROCm Archlinux Wiki](https://wiki.archlinux.org/title/GPGPU#ROCm)
- [ROCm Repo](https://github.com/rocm-arch/rocm-arch)

#### LMStudio

- [LMStudio](https://lmstudio.ai/)
- [LMStudio Docs](https://lmstudio.ai/docs/welcome)
- [LMStudio Repo](https://github.com/lmstudio-ai)

#### Ollama

- [Ollama](https://ollama.com/)
- [Ollama Blog AMD](https://ollama.com/blog/amd-preview)
- [Ollama Repo](https://github.com/ollama/ollama)
- [Ollama Docs](https://github.com/ollama/ollama/tree/main/docs)

#### Jan AI

- [Jan AI](https://jana.ai/)
- [Jan AI Docs](https://jan.ai/docs)
- [Jan AI Repo](https://github.com/janhq/jan)
